...
I can also simplify my `monad` input down to just the important numbers for each digit of the model number:
...
This will be run much faster than my previous `check_model_number` function, but doesn't solve the main problem: it requires checking every number until `z = 0`.
...
I'm not very familiar with these terms as a non-CS grad, but apparently this mimics a [stack data structure](https://en.wikipedia.org/wiki/Stack_(abstract_data_type) which can *push* a value (adds it to the stack) or *pop* the most recent (remove from the stack).
...
My input corresponds to the following push an pop operations on the 14 digits of a model number:
...
Then the maximum valid model number doesn't require any programming, just some basic logic and picking the maximum possible digit for each:
...
```{r}
day25 <- read_lines(here("_posts", "2021-12-21-advent-of-code-2021-days-21-25",
                         "day25-input.txt"))
day25 <- read_lines(here("_posts", "2021-12-21-advent-of-code-2021-days-21-25",
                         "day25-example.txt"))
...
I really lagged behind in the last few days, mostly due to work and spending time with family, but I also found the puzzles more difficult.
This is probably a similar story to others on the private leaderboard:

```{r eval=FALSE, echo=FALSE}
Sys.setenv(
  AOC_COOKIE = "53616c7465645f5fbe570e13f71d70a18d97192d755bd2476a103db08d06ce8afc8fe162aee4d2bbecfbd75a9e9f2e65"
)
```
...
leaderboard %>%
  write_rds(here("_posts", "2021-12-21-advent-of-code-2021-days-21-25",
                 "leaderboard_2021-12-25.rds"))
```

```{r echo=FALSE}
leaderboard <-
  read_rds(here("_posts", "2021-12-21-advent-of-code-2021-days-21-25",
                "leaderboard_2021-12-25.rds"))
```
...
13th place out of 126, which I'm very happy with.
Especially considering that the puzzles released at 1AM my time, so I generally didn't get to start until after work the next day.
