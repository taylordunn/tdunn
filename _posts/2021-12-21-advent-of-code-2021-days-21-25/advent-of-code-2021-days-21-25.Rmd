...
But with 9^14 = `r scales::comma(9^14)` possible numbers to check, I doubt the brute force approach would finish in any reasonable time frame.
...
```{r}
monad_simple_step <- function(w, z = 3, step5 = 1, step6 = 10, step16 = 10) {
  # Step 2
  x <- 0
  # Step 3
  x <- z
...
monad_simple_step(5)
monad_simple_step(3, z = 8, step5 = 26, step6 = -12, step16 = 4)
```
...
check_model_number_simple <- function(model_number) {
  m <- monad_simple %>%
    mutate(w = strsplit(model_number, "")[[1]])
  
  reduce(transpose(m), ~paste0(.$w, collapse = "_"))
  
  reduce2(model_number, monad_simple$input_step, paste0) 
...
run_monad_simple <- function(model_number) {
