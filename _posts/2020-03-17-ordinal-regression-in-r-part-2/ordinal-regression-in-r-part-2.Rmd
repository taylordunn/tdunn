...
These are essentially equal, so the weak priors have not made a large difference to the model fit.
...
`brms` has a convenient function `conditional_effects` for quickly plotting estimates.
...
The `brms` package is an awesome tool for fitting Bayesian models in Stan.
Though it requires a more thoughtful (what are my priors?) and slow (Markov chain Monte Carlo can take a long time) approach, I find Bayesian inference *far more* intuitive than frequentest null hypothesis significance testing.
