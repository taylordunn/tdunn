...
```{r eval=FALSE}
max(c(day5_df$x1, day5_df$x2))
max(c(day5_df$y1, day5_df$y2))

ocean_floor <- matrix(0, nrow = 1000, ncol = 1000)

for (line in day5_df$xy) {
  ocean_floor[unique(line$x), unique(line$y)] <-
    ocean_floor[unique(line$x), unique(line$y)] + 1
}
sum(ocean_floor > 1)
```
...
bind_rows(
  day5_straight %>% select(xy) %>% unnest(xy),
  day5_diag %>% select(x, y) %>% unnest(c(x, y))
) %>%
  count(x, y) %>%
  summarise(dangerous_points = sum(n > 1))
```
