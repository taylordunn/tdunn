...
params:
  date: 2021-12-28
  slug: "2021-12-28-canada-covid-19-data-in-r-exploring-the-api"
categories:
  - R
  - COVID-19
  - API
...
There aren't any missing `date`s in this report, so the `fill_dates` parameter makes no difference here.
...
```{r}
vaccine_age_groups$data[[1]] %>% str_trunc(90)
```
...
A specific age group can also be queries with the `group` parameter.
...
A helpful variable is the `data_status`, which indicates if the daily numbers have been reported:
