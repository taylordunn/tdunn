...
The polynomial of degree 5 did best here, which I'll use to finalize the workflow and fit to the full training set:
...
We see that `n_bikes_lag_14`, `mean_temperature`, `total_precipitation` and `site_name` are important with all three models.
...
I'll choose a model by the best MASE:
...
The Vernon St site has a series of zeroes in a row that I'm guessing aren't real (see aside).

I also think some of the poor predictions can be explained by missing data.
For instance, there are a few days of poor predictions around 2022-11-23, where the model is over-estimating `n_bikes` at a few sites.
