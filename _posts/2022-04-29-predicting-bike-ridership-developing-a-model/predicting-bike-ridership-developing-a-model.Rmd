...
bike_recipe_date <-
  ...
  #step_date(count_date, features = c("dow", "doy", "decimal"),
...
bike_recipe2 <-
  ...
  update_role(count_date, new_role = "date_variable") %>%
  #add_role(count_date, new_role = "date_variable") %>%
  #step_date(count_date, features = c("dow", "doy", "decimal", "year"),
  step_date(count_date, features = c("dow", "doy", "year"),
            label = TRUE, ordinal = FALSE) %>%
  step_ns(count_date_doy, deg_free = 5) %>%
  step_mutate(count_date_dec = lubridate::decimal_date(count_date)) %>%
  #step_mutate(count_date_dec = as.numeric(count_date)) %>%
  step_zv(all_predictors())
```
...
test_wf_set <- workflow_set(
  preproc = list(rec1 = bike_recipe1, rec2 = bike_recipe2),
...
glm_wf_set <- workflow_set(
  preproc = list(base = bike_recipe,
                 ...
                 date_weather = bike_recipe_date_weather, test = bike_recipe_test),
...
bike_tree_recipe_dummy <- bike_tree_recipe %>%
  step_dummy(all_nominal_predictors())
```
...
trees_wf_set <- workflow_set(
  #preproc = list(tree_recipe = bike_tree_recipe),
  preproc = list(tree_recipe = bike_tree_recipe, tree_recipe = bike_tree_recipe,
                 tree_recipe_dummy = bike_tree_recipe_dummy),
...
plot_wf_set_metrics(wf_set_2_res)
autoplot(wf_set_2_res)
autoplot(wf_set_2_res)
```
...
```{r}
wf_set_2_res %>%
  extract_workflow_set_result("tree_recipe_dummy_xgb") %>%
  collect_notes() %>%
  pull(note) %>%
  .[[1]]
  .$note[[1]]
...
```{r}
xgb_recipe <- bike_tree_recipe_dummy %>%
  step_rm(count_date)
xgb_wf <- workflow() %>%
  add_recipe(xgb_recipe) %>%
  add_model(boost_tree(mode = "regression"))
fit(xgb_wf, data = bike_train)
```
...
```{r}
rank_results(wf_set_2_res, select_best = F, rank_metric = "mase") %>%
  filter(.metric == "mase") %>%
  select(rank, wflow_id, .config, mean, std_err) %>%
  gt() %>%
  fmt_number(columns = c(mean, std_err), decimals = 3)
...
```{r}
extract_workflow_set_result(wf_set_2_res, "weather_date_ranger") %>%
  autoplot() +
  dunnr::add_facet_borders()
```
...
# Add a non-numeric ID variable
Chicago <- Chicago %>% dplyr::mutate(id = as.character(1:n()))
