...
It might make more sense to stratify by `site_name` so that there is a 70-30 split in each site (instead of splitting into the first 70% and last 30% of the full data).
...
I'll use this simpler approach for now.
...
```{r}
bike_metrics <- metric_set(rmse, mape, rsq, poisson_log_loss)
```

The Poisson log loss is a new one to me -- it is appropriate for modeling count data, which we have here.
...
An extension of this model will include date variables of day of week, month, year and holidays (using the Canadian holidays I showed previously):
...
To make sure everything is working as expected, run the training set through this pre-processing recipe:

```{r}
prep(bike_recipe_date) %>%
  bake(bike_train) %>%
  glimpse()
```
...
```{r}
wf_set_1_fits <-
  workflow_map(wf_set_1, "fit_resamples", verbose = TRUE,
               resamples = bike_resamples, metrics = bike_metrics)
```

```{r}
wf_set_1_fits$result[[1]] %>%
  collect_notes() %>%
  pull(note)
```
...
wf1 <- workflow() %>%
  ...
  add_model(lm_spec)
test_fits <- fit_resamples(
  wf1, resamples = bike_resamples,
  control = control_resamples(save_workflow = T, save_pred = T, verbose = T)
)
test_fits %>%
  collect_notes() 
...
```{r}
bike_analysis_6 <- analysis(bike_resamples$splits[[6]])
bike_assess_6 <- assessment(bike_resamples$splits[[6]])

bike_fit_6 <- fit(wf1, bike_analysis_6)
predict(bike_fit_6, bike_assess_6)
```
