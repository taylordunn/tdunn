...
bike_ranger_recipe <-
  ...
  step_mutate_at(c(total_precipitation, snow_on_ground),
                 fn = ~ replace_na(., 0)) %>%
  step_zv(all_predictors())
bike_ranger_recipe_test <-
  recipe(n_bikes ~ count_date + site_name + n_bikes_lag_14 + mean_temperature +
           total_precipitation + speed_max_gust + snow_on_ground,
         data = bike_train) %>%
  step_string2factor(site_name) %>%
  add_role(count_date, new_role = "date_variable") %>%
  step_date(count_date, features = c("dow", "doy", "year"),
            label = TRUE, ordinal = FALSE) %>%
  step_holiday(count_date, holidays = canada_holidays) %>%
...
wf_set_2 <- workflow_set(
  preproc = list(weather_date = bike_ranger_recipe),
  models = list(ranger = rf_spec),
  cross = TRUE
)
```
...
wf_set_2_res <- workflow_map(
  wf_set_2,
  "tune_grid",
  # The rand_forest() tuning defaults are usually sufficients,
  #  so just try 15 candidate parameter sets
  grid = 15,
  resamples = bike_resamples,
  metrics = bike_metrics, verbose = TRUE
)
...
ranger_recipe <- 
  recipe(
    formula = n_bikes ~ count_date + site_name + n_bikes_lag_14 + mean_temperature + 
