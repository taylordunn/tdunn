...
In [my last post](../2022-04-27-predicting-bike-ridership-getting-the-data), I retrieved and prepared bike counter data from Halifax, Nova Scotia, and some historical weather data to go along with it.
...
autoplot(bike_glm_wf_set_res, metric = "rmse")
```
...
Take a closer look by truncating the dates:
...
The worst time period for prediction seems to be around 2022-11-23.
...
Since this model used a Poisson model, the link function (non-linear relationship between the outcome and the predictors) is the logarithm:
...
Visualize the performance 30 workflows, ranked by MASE:
...
The visualization (done with the `rpart.plot` package) is hidden below:
...
We just have one model and one preprocessor now, so put it into a `workflow`:
