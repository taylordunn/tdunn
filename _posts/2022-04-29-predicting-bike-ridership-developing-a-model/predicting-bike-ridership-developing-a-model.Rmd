...
mod <- bike_glm_fit$.workflow[[1]] %>% extract_fit_engine()
d <- bike_train %>%
  mutate(
    count_date_year = lubridate::year(count_date),
    count_date_doy = lubridate::yday(count_date),
    count_date_dow = lubridate::wday(count_date, label = TRUE)
  )
mod2 <- glm(
  n_bikes ~ count_date + n_bikes_lag_14 + mean_temperature + total_precipitation +
    speed_max_gust + snow_on_ground + count_date_year + count_date_doy +
    count_date_dow + site_name,
  data = d, family = poisson
)
marginalmeans(mod, variables = "mean_temperature")

bike_glm_me <- bike_glm_fit$.workflow[[1]] %>%
  extract_fit_engine() %>%
  marginaleffects(variables = "total_precipitation")
  #marginaleffects()
summary(bike_glm_me, by = "site_name")
```
...
bike_glm_fit$.workflow[[1]] %>%
  extract_fit_engine() %>%
  summary()
gtsummary::tbl_regression(bike_glm_fit$.workflow[[1]]) %>%
  gtsummary::as_gt() %>%
  gt::tab_options(container.overflow.y = TRUE, container.height = 300)
```
...
For tree-based methods, I'll use the same pre-processing except I won't use a spline term (trees partition the feature space to capture non-linearity):
...
I'll try a decision tree, a random forest, and an XGBoost model.
...
As a first pass, I won't be very careful with my hyperparameter tuning.
...
trees_wf_set_res <- workflow_map(
  trees_wf_set,
  "tune_grid",
  # Just try 10 candidate parameter sets for each model (for now)
  grid = 10,
  resamples = bike_resamples,
...
```{r}
extract_workflow_set_result(trees_wf_set_res, "tree_recipe_dummy_xgb") %>%
  #autoplot(metric = "mase")
  select_best(metric = "mase")
```

```{r}
bike_trees_workflow <- extract_workflow(trees_wf_set, "tree_recipe_dummy_xgb")
bike_trees_workflow_best <-
  extract_workflow_set_result(trees_wf_set_res, "tree_recipe_dummy_xgb") %>%
  select_best(metric = "mase")

bike_trees_workflow_final <- finalize_workflow(
  bike_trees_workflow, bike_trees_workflow_best
)
bike_trees_fit <- last_fit(
  bike_trees_workflow_final, split = bike_split, metrics = bike_metrics
)
collect_metrics(bike_trees_fit)
```
...
xgb_grid <- grid_latin_hypercube(
  ...
  finalize(mtry(), bike_train_baked),
  learn_rate(),
  size = 30
)
```
