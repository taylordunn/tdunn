...
autoplot(glm_wf_set_res, metric = "rmse")
```
...
bike_glm_workflow <-
  extract_workflow(glm_wf_set_res, id = "date_weather_poisson_reg")
bike_glm_tune_res <-
  extract_workflow_set_result(glm_wf_set_res, id = "date_weather_poisson_reg")
...
```{r include=FALSE}
weather_example <- bike_glm_fit$.workflow[[1]] %>%
  tidy() %>%
  filter(str_detect(term, "temperature|precip|max_gust|snow")) %>%
...
I'll try a decision tree, a random forest, and an XGBoost model, each with a number of hyperparameters indicated for tuning:
...
The boosted tree models (which also consist of multiple decision trees, but differ in how they are built and combined) slightly outperform random forests, which is also unexpected. 
...
bike_rf_fit <- last_fit(
  finalize_workflow(rf_spec, select_best(bike_rf_wf_res, "mase")),
  split = bike_split, metrics = bike_metrics
)
bike_rf_fit <- last_fit(
  finalize_workflow(set_engine(bike_rf_wf, "ranger", importance = "permutation"),
                    select_best(bike_rf_wf_res, "mase")),
  split = bike_split, metrics = bike_metrics
)
...
```{r}
autoplot(bike_decision_wf_res) + geom_line()
autoplot(bike_rf_wf_res) + geom_line()
autoplot(bike_xgb_wf_res) + geom_line()
```
...
The holiday features are negligible in this model.
...
# Get the number of predictors so I can set number of predictors in `mtry()`
#bike_train_baked <- prep(bike_trees_recipe_dummy) %>%
bike_train_baked <- prep(bike_trees_recipe) %>%
  bake(bike_train)
...
bike_xgb_recipe2 <-
  ...
  step_holiday(count_date, holidays = canada_holidays) %>%
  step_novel(all_nominal_predictors()) %>%
  step_indicate_na(speed_max_gust, total_precipitation, snow_on_ground) %>%
  step_impute_mean(speed_max_gust) %>%
  step_mutate_at(c(total_precipitation, snow_on_ground),
                 fn = ~ replace_na(., 0)) %>%
  step_mutate(count_date = as.numeric(count_date)) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_zv(all_predictors())
bike_xgb_recipe3 <-
  recipe(n_bikes ~ count_date + site_name + n_bikes_lag_14 + mean_temperature +
...
bike_xgb_wf_set <- workflow_set(
  preproc = list(rec1 = bike_xgb_recipe1, rec2 = bike_xgb_recipe2),
                 #rec3 = bike_xgb_recipe3),
...
set.seed(8990)
tic()
bike_xgb_wf_set_res <- workflow_map(
  bike_xgb_wf_set,
  "tune_grid",
  grid = xgb_grid, resamples = bike_resamples, metrics = bike_metrics,
  verbose = TRUE
)
toc()
```
...
```{r}
workflowsets::collect_metrics(bike_xgb_wf_set_res) %>%
  filter(.metric == "mase") %>%
  group_by(wflow_id) %>%
  slice_min(mean, n = 5)
```
...
```{r}
wf_1 <- extract_workflow(bike_xgb_wf_set_res, "rec1_boost_tree")
wf_2 <- extract_workflow(bike_xgb_wf_set_res, "rec2_boost_tree")
#wf_3 <- extract_workflow(bike_xgb_wf_set_res, "rec3_boost_tree")
wf_res_1 <- extract_workflow_set_result(bike_xgb_wf_set_res, "rec1_boost_tree")
wf_res_2 <- extract_workflow_set_result(bike_xgb_wf_set_res, "rec2_boost_tree")
#wf_res_3 <- extract_workflow_set_result(bike_xgb_wf_set_res, "rec3_boost_tree")
wf_fit_1 <- last_fit(
  finalize_workflow(wf_1, select_best(wf_res_1, metric = "mase")),
  split = bike_split, metrics = bike_metrics
)
wf_fit_2 <- last_fit(
  finalize_workflow(wf_2, select_best(wf_res_2, metric = "mase")),
  split = bike_split, metrics = bike_metrics
)
# wf_fit_3 <- last_fit(
#   finalize_workflow(wf_3, select_best(wf_res_3, metric = "mase")),
#   split = bike_split, metrics = bike_metrics
# )

collect_metrics(wf_fit_1)
collect_metrics(wf_fit_2)
#collect_metrics(wf_fit_3)
```


```{r}
plot_wf_set_metrics(bike_xgb_wf_set_res)
autoplot(bike_xgb_wf_set_res) +
  geom_point(aes(color = preprocessor))
```


We can "peer into the black box" so-to-speak with `autoplot()`
...
```{r}
show_best(bike_xgb_tune, metric = "mase")
select_best(bike_xgb_tune, metric = "mase")

bike_xgb_fit <- last_fit(
  finalize_workflow(bike_xgb_workflow,
                    select_best(bike_xgb_tune, metric = "mase")),
  split = bike_splits, metrics = bike_metrics
)
select_best(bike_xgb_tune, metric = "mase")
```
...
```{r}
collect_metrics(svm_wf_set_res) %>%
  filter(.metric == "mase") %>%
  arrange(mean)
```

```{r}
bike_svm_wf <- extract_workflow(svm_wf_set, "recipe_poly")
bike_svm_wf_res <- extract_workflow_set_result(svm_wf_set_res, "recipe_poly")

autoplot(bike_svm_wf_res)
```
...
kernlab_recipe <- 
  recipe(
    n_bikes ~ count_date + site_name + n_bikes_lag_14 + mean_temperature + 
    total_precipitation + speed_max_gust + snow_on_ground, data = bike_train
  ) %>%
  step_zv(all_predictors()) %>% 
  step_normalize(all_numeric_predictors()) 
...
```{r}
usemodels::use_ranger(n_bikes ~ site_name + n_bikes_lag_14, data = bike_train, clipboard = T, colors = FALSE)
ranger_recipe <- 
