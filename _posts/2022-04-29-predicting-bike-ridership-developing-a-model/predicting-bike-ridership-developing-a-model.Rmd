...
For tree-based methods, I'll use the same pre-processing except I won't use a spline term, and won't use dummy variables (not needed with these methods):
...
wf_set_2 <- workflow_set(
  preproc = list(weather_date = bike_tree_recipe),
  models = list(rf = rf_spec, decision = decision_spec),
  cross = TRUE
)
```
...
wf_set_2_res <- workflow_map(
  wf_set_2,
  "tune_grid",
  # Just try 15 candidate parameter sets for each model (for now)
  grid = 15,
  resamples = bike_resamples,
