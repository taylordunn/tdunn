...
In [my last post](../2022-04-27-predicting-bike-ridership-getting-the-data), I retrieved and prepared bike counter data from Halifax, Nova Scotia.
I also got some historical weather data to go along with it.
Here, I will explore the data, engineer some features, and fit and evaluate many models to predict bike ridership at different sites around the city.
...
[All except the Hollis St site have two channels](../2022-04-27-predicting-bike-ridership-getting-the-data/#eda-and-cleaning) (northbound and southbound), which is why `n_records` = 48. The entries with fewer `n_records` reflect the time of day that the data was extracted:
...
This will require imputation of missing values for all 4 variables, which I'll attempt here.
...
### Splitting, resampling and metrics
...
Trying the aside again but with comments.
...
An extension of this recipe will include date variables.
...
bike_recipe4 <- bike_recipe1 %>%
  step_mutate(
    count_date = lubridate::year(count_date) + lubridate::yday(count_date)/365
  )

test_wf_set <- workflow_set(
  preproc = list(rec1 = bike_recipe1, rec2 = bike_recipe2, rec3 = bike_recipe3,
                 rec4 = bike_recipe4),
...
Define the model specifications and put everything into a `workflow_set()`:
...
```{r bike-glm-wf-set-res, cache=TRUE}
tic()
bike_glm_wf_set_res <- workflow_map(
  bike_glm_wf_set,
  "tune_grid",
...
```{r}
rank_results(bike_glm_wf_set_res) %>%
...
plot_wf_set_metrics(bike_glm_wf_set_res)
```
...
```{r}
rank_results(bike_glm_wf_set_res, select_best = TRUE, rank_metric = "mase") %>%
  filter(.metric == "mase") %>%
...
So our best workflow has the id `date_weather_poisson_reg`^[You could make the argument that the improvement is so minor that it is worth choosing the simpler linear regression model instead.] with the `.config` "Preprocessor2_Model1" which refers to a specific spline degree from our tuning of the `count_date_doy` feature.
Extract this workflow and checkout the results of the tuning:

```{r fig.height=3, fig.width=6}
bike_glm_workflow <-
  extract_workflow(bike_glm_wf_set_res, id = "date_weather_poisson_reg")
bike_glm_tune_res <-
  extract_workflow_set_result(bike_glm_wf_set_res,
...
Here are the predictions overlaid on the data (vertical line delineates training and testing sets):

```{r}
bike_glm_preds <- bind_rows(
...
The worst-performing region looks to be Hollis St in 2020.
In contrast, I'm impressed by how well the bimodal counts were captured for South Park St.
...
That looks pretty good to me, though it's a shame to not have more data to test a full year -- most of the test set covers winter and spring.
...
worst_preds %>%
  ...
  unnest(count_date) %>%
  left_join(bike_glm_preds, by = c("data_set", "site_name", "count_date")) %>%
...
Relative to the noise in the data, that doesn't look bad at all.
The Vernon St site has a series of zeroes in a row that I'm guessing aren't real.
The worst time period for prediction seems to be around 2022-11-23, where I'm over-estimating `n_bikes` for 3 of the sites.
Check out the weather around that day:

```{r}
bike_ridership %>%
  filter(count_date > "2021-11-15", count_date < "2021-11-26") %>%
  distinct(count_date, mean_temperature, total_precipitation,
           speed_max_gust, snow_on_ground) %>%
  gt()
```
...
If I impute a large amount of rain (let's say 30mm) for that date, here is how the predictions change:

```{r}
augment(
  bike_glm_workflow_final,
  bike_glm_preds %>%
    filter(count_date == "2021-11-23") %>%
    rename(.pred_old = .pred) %>%
    mutate(total_precipitation = 30)
) %>%
...
```{r}
extract_fit_parsnip(bike_glm_fit) %>%
  tidy() %>%
...
```{r include=FALSE}
weather_example <- extract_fit_parsnip(bike_glm_fit) %>%
  tidy() %>%
  filter(str_detect(term, "temperature|precip|max_gust|snow")) %>%
...
bike_trees_recipe_dummy <- bike_trees_recipe %>%
  step_dummy(all_nominal_predictors())
```
...
xgb_spec <- boost_tree(mtry = tune(), min_n = tune(), tree_depth = tune(),
...
trees_wf_set <- workflow_set(
  preproc = list(trees_recipe = bike_trees_recipe,
                 trees_recipe = bike_trees_recipe,
                 trees_recipe_dummy = bike_trees_recipe_dummy),
...
trees_wf_set_res <- workflow_map(
  ...
  "tune_grid",
  grid = 10,
  resamples = bike_resamples, metrics = bike_metrics, verbose = TRUE
)
toc()
```
...
As I would have expected, the decision tree models perform worse than random forest models (which consist of multiple decision trees).
...
For the three different tree-based model types, I'll extract the best-performing workflow from each and fit to the test set:

```{r}
bike_decision_wf <- extract_workflow(trees_wf_set, "trees_recipe_decision")
bike_decision_wf_res <-
  extract_workflow_set_result(trees_wf_set_res, "trees_recipe_decision")

bike_decision_fit <- last_fit(
  finalize_workflow(bike_decision_wf,
                    select_best(bike_decision_wf_res, "mase")),
  split = bike_split, metrics = bike_metrics
)

bike_rf_wf <- extract_workflow(trees_wf_set, "trees_recipe_rf")
bike_rf_wf_res <-
  extract_workflow_set_result(trees_wf_set_res, "trees_recipe_rf")

# I forgot to add the `importance` argument in the original spec,
#  but I can update the model spec for the `last_fit()`
# This will be needed for computing variable importance
bike_rf_wf <- bike_rf_wf %>%
  update_model(set_engine(rf_spec, "ranger", importance = "permutation"))
bike_rf_fit <- last_fit(
  finalize_workflow(bike_rf_wf, select_best(bike_rf_wf_res, "mase")),
  split = bike_split, metrics = bike_metrics
)
bike_rf_fit <- last_fit(
  finalize_workflow(bike_rf_wf, select_best(bike_rf_wf_res, "mase")),
...
bike_xgb_wf <- extract_workflow(trees_wf_set, "trees_recipe_dummy_xgb")
bike_xgb_wf_res <-
  extract_workflow_set_result(trees_wf_set_res, "trees_recipe_dummy_xgb")

bike_xgb_fit <- last_fit(
  finalize_workflow(bike_xgb_wf,
                    select_best(bike_xgb_wf_res, "mase")),
  split = bike_split, metrics = bike_metrics
)
```

And compile the metrics on the test set:

```{r}
bind_rows(
  decision_tree = collect_metrics(bike_decision_fit),
  rand_forest = collect_metrics(bike_rf_fit),
  xgb = collect_metrics(bike_xgb_fit),
  .id = "model"
) %>%
...
The random forest actually outperformed the XGBoost model here, but I'll see later if I can improve on this with more thorough tuning.

#### Model interpretation

```{r include=FALSE}
# Get some statistics for the decision tree
rpart_fit <- extract_fit_engine(bike_decision_fit)
...
We see that `n_bikes_lag_14`, `mean_temperature` and `total_precipitation` are important with all three.
Measures of time within (`count_date_doy`) and across (`count_date_year`) years are also important

Investigate the distribution of predictions:

```{r fig.height=3, fig.width=6}
bind_rows(
  decision_tree = collect_predictions(bike_decision_fit),
  rand_forest = collect_predictions(bike_rf_fit),
  xgb = collect_predictions(bike_xgb_fit),
  .id = "model"
) %>%
  ggplot(aes(x = .pred)) +
...
The distribution of predictions is much smoother in the ensemble methods (random forest and XGBoost) which isn't surprising.
...
So far, I've only considered 10 candidate sets of hyperparameters for tuning the XGBoost model.
Let's try 100:

```{r}
# Re-define the XGB spec with trees = tune()
xgb_spec <- boost_tree(
...
bike_train_baked <- prep(bike_trees_recipe) %>% bake(bike_train)
...
We have just one model and one preprocessor now, so put it into a `workflow`:

```{r}
bike_xgb_workflow <- workflow() %>%
  add_recipe(bike_trees_recipe_dummy) %>%
  add_model(xgb_spec)
```

And tune:

```{r bike-xgb-tune, cache=TRUE}
set.seed(2081)
tic()
bike_xgb_tune <- tune_grid(
  bike_xgb_workflow, resamples = bike_resamples,
  grid = xgb_grid, metrics = bike_metrics
)
toc()
```
Here is the `mase` for the different candidate models:

```{r}
autoplot(bike_xgb_tune, metric = "mase")
```

Take the best model and fit to the test set:

```{r}
bike_xgb_fit <- last_fit(
  finalize_workflow(bike_xgb_workflow,
                    select_best(bike_xgb_tune, metric = "mase")),
  split = bike_split, metrics = bike_metrics
)
collect_metrics(bike_xgb_fit)
```

All that effort and I couldn't get better performance than simple Poisson regression!

```{r}
select_best(bike_xgb_wf_res, "mase")
select_best(bike_xgb_tune, "mase")
```
...
svm_wf_set <- workflow_set(
  preproc = list(bike_svm_recipe),
  models = list(linear = svm_linear_spec, poly = svm_poly_spec,
                rbf = svm_rbf_spec)
)
```

```{r svm-wf-set-res, cache=TRUE}
set.seed(4217)
tic()
...
Those are some pretty interesting (and smooth) trends, with separation between two distinct levels of performance.
The best model is one of the `svm_rbf` candidates (though the next 10 are the `svm_poly` models).
Evaluate it on the test set:

```{r}
bike_svm_wf <- extract_workflow(svm_wf_set, "recipe_rbf")
bike_svm_wf_res <- extract_workflow_set_result(svm_wf_set_res, "recipe_rbf")

bike_svm_fit <- last_fit(
  finalize_workflow(bike_svm_wf, select_best(bike_svm_wf_res, "mase")),
  split = bike_split, metrics = bike_metrics
)
collect_metrics(bike_svm_fit)
```
...
# A quick ARIMA fit for posterity
library(modeltime)
...
bike_arima_workflow <- workflow() %>%
  add_model(arima_spec) %>%
  add_recipe(bike_arima_recipe)

bike_arima_res <- fit_resamples(
  bike_arima_workflow, resamples = bike_resamples, metrics = bike_metrics
)

collect_metrics(bike_arima_res)

last_fit(
  bike_arima_workflow, split = bike_split, metrics = bike_metrics
) %>%
  collect_metrics()
```
...
```{r}
bind_rows(
  rank_results(bike_glm_wf_set_res, rank_metric = "mase",
                     select_best = TRUE) %>%
  ...
  collect_metrics(bike_xgb_wf_res) %>%
  ...
  arrange(mase) %>%
  gt() %>%
  fmt_number(c(mase, std_err), decimals = 3)
```
