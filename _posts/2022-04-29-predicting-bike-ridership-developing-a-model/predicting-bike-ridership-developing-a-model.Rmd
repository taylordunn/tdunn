...
bike_recipe_weather <-
  ...
  step_mutate_at(c(total_precipitation, snow_on_ground),
                 fn = ~ replace_na(., 0)) %>%
  # step_mutate(
  #   total_precipitation = ifelse(is.na(total_precipitation), 0, total_precipitation)
  # ) %>%
  # step_mutate(
  #   snow_on_ground = replace_na(snow_on_ground, 0)
  # ) %>%
  step_novel(all_nominal_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_zv(all_predictors())
...
* Poisson regression performed worse than linear regression. This isn't too surprising because of the previously mentioned over-dispersion. I'd like to re-visit this approach when the negative binomial model becomes available.
* Linear regression generally performed better than Poisson regression.

```{r eval=FALSE}
extract_workflow_set_result(wf_set_1_res, "date_weather_poisson_reg") %>%
  autoplot()
wf <- extract_workflow(wf_set_1_res, "date_weather_poisson_reg")

wf_res <- tune_grid(wf, resamples = bike_resamples, metrics = bike_metrics, 
                    grid = grid_regular(deg_free(c(3, 7)), levels = 5),
...
```{r}
rank_results(wf_set_1_res)
autoplot(wf_set_1_res, id = "dates_linear_reg")
best_wf_res <- extract_workflow_set_result(wf_set_1_res, id = "dates_linear_reg")
best_wf <- extract_workflow(wf_set_1_res, id = "dates_linear_reg")
best_wf_final <- finalize_workflow(
  best_wf,
  select_best(best_wf_res, metric = "rmse")
)
best_wf_final_test_fit <- fit(best_wf_final, bike_train)

best_wf_final_fit <- last_fit(best_wf_final, bike_split)
collect_metrics(best_wf_final_fit)
```

```{r fig.height=5, fig.width=5}
augment(best_wf_final_test_fit, bike_train) %>%
...
```{r}
bike_metrics(best_wf_final_test_fit)
augment(best_wf_final_test_fit, bike_train) %>%
  bike_metrics(n_bikes, .pred)
augment(best_wf_final_test_fit, bike_test) %>%
  bike_metrics(n_bikes, .pred)
```
