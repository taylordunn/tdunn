...
3. `usethis::use_pipe`. As ` tidyverse` enthusiast, I always setup my packages to use `magittr`'s pipe operator `%>%`.
...
6. Write tests with `testthat`.
    * `use_test("file-name")` creates the `tests/testthat/file-name.R` file, paired to `R/file-name.R` file.
...
* `match.arg` is a helpful base R function which matches the `split` parameter to just one of the given values. It returns an error is an unexpected value is provided.
...
I might alter this function in the future to send these requests slower, either with a simple `Sys.sleep()` function or the [`polite` package](https://github.com/dmi3kno/polite).
