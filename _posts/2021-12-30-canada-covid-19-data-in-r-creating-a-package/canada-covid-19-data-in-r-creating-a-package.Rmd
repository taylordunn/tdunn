...
Pushing the `reports` file and tests to GitHub resulted in failed `R CMD check`s on all platforms, with the HTTP code 429.
This is because all my `testthat` tests were running on 9 different platforms and flooding the API with too many requests.

There are probably more sophisticated ways to limit my request rate (e.g. using the [re-written `httr2` package](https://httr2.r-lib.org/), the [`vcr` package](https://github.com/vcr/vcr), the [`httptest` package](https://github.com/nealrichardson/httptest)) but I decided to use the humble `Sys.sleep()`.
...
```{r code_folding="get_vaccination_data documentation"}
...
Because this function can return both summaries and reports, I have to put in `stop()` condition if reports by health region is requested (not allowed by the API).

The tests for `get_vaccination_data` make use of my new `request_sleep` function:
...
Another `stop` condition prevents sending too many requests: this time requesting all data for all subregions.
...
These data are week-by-week vaccination statistics by various age groups.
The request URL is `/vaccines/age-groups` and takes a few parameters:

```{r code_folding="get_vaccine_age_groups documentation"}
...
There were some tricky aspects to this function, like how the content is JSON within JSON (see my [previous post](../2021-12-28-canada-covid-19-data-in-r-exploring-the-api/#vaccine-age-groups)).
...
The last function I added to this first iteration of the package is `get_provinces`, which has some population/geographical data, as well as a [`data_status` variable which indicates if a province's daily numbers have been updated](https://tdunn.ca/posts/2021-12-28-canada-covid-19-data-in-r-exploring-the-api/#provinces):
