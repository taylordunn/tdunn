...
I can get a list of available data sets (which they call collections) as follows:
^[To build these API queries, I found [this documentation](https://api.weather.gc.ca/openapi?f=html#/) to be very helpful.]

```{r resp-collections, cache=TRUE}
...
```{r resp-climate-stations-1, cache=TRUE}
...
```{r resp-climate-stations-2, cache=TRUE}
...
```{r resp-climate-dockyard, cache=TRUE}
resp <- GET(
  paste0(
    base_url,
...
```{r resp-climate-windsor, cache=TRUE}
resp <- GET(
  paste0(base_url,
...
# A convenience function for quickly extracting a station's data
...
Quite a bit of missing data, but we should have enough to make an interesting model.
Save the data:
