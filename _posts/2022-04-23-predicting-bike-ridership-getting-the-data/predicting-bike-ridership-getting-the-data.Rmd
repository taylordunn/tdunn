...
I'll get this from directly from [the Meteorological Service of Canada's API](https://api.weather.gc.ca/).
...
```{r}
content_parsed <- content(resp, as = "text")
test <- jsonlite::fromJSON(content_parsed)
...
The README for the climate data is located here: https://eccc-msc.github.io/open-data/msc-data/climate_obs/readme_climateobs_en/.

The raw data is located here: https://dd.weather.gc.ca/climate/observations/.

```{r}
climate_station_list <- read_csv("https://dd.weather.gc.ca/climate/observations/climate_station_list.csv")
glimpse(climate_station_list)
```
