...
```{r}
day7_sample <- strsplit("16,1,2,0,4,2,7,1,2,14", ",") %>%
  unlist() %>% as.numeric()
day7_sample
mean(day7_sample); median(day7_sample)
```

### Part 1

```{r}
day7 <- strsplit(day7, ",") %>% unlist() %>% as.numeric()
median(day7)
```
...
map_int(
  abs(day7_sample - median(day7_sample)),
  ~ifelse(
    median(median(day7_sample) == .x), 0L,
    cumsum(1:.x) %>% tail(1)
  )
)

for (test_point in seq(1:5)) {
  print(test_point)
  
  fuel_spent <- map_int(
    abs(day7_sample - test_point),
    ~ifelse(
      .x == 0, 0L,
      cumsum(1:.x) %>% tail(1)
    )
  ...
  print(sum(fuel_spent))
}
```


```{r}
fuel_spent_list <- list()
min_point <- median(day7) + 100
max_point <- median(day7) + 400
for (test_point in seq(min_point, max_point)) {
  fuel_spent <- map_int(
    abs(day7 - test_point),
    ~ifelse(
      .x == 0, 0L,
  ...
  fuel_spent_list[as.character(test_point)] <- sum(fuel_spent)   
}

tibble(test_point = names(fuel_spent_list),
       fuel_spent = as.numeric(fuel_spent_list)) %>%
  arrange(fuel_spent)
