...
```{r}
ikea <- tt$ikea %>% rename(row_number = X1)
glimpse(ikea)
```
...
I'll start with a simple linear regression, which I expect will be outperformed by a more flexible method.
^[See ISLR section 2.2.2 [@James2013] on the bias-variance trade-off for an accessible explanation of this idea.]
