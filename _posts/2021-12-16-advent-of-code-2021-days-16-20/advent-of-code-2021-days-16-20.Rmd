...
```{r}
scanners_overlap$overlap[[1]]
```

```{r}
d1 %>% select(i, i2, dist2)

d1 <- scanners_overlap$overlap[[1]]
d2 <- scanners_overlap$overlap[[1]]
d2
```


Then the beacons in common between scanner 0 and 1 are:

```{r}
common_beacons <- scanners_overlap %>%
  filter(scanner == 0, scanner2 == 1) %>%
  unnest(overlap) %>%
  transmute(scanner, scanner2, label = str_c("")
            i, i2, j, j2) %>%
  pivot_longer(cols = i:j2) %>%
...
```{r}
common_beacons <- scanners_overlap %>%
  group_by(scanner, scanner2, i) %>%
  nest() %>%
  mutate(
    j = map_chr(data,
                ~sort(table(c(.x$j, .x$j2))) %>% tail(1) %>% names()),
    j = as.integer(j)
  ) %>%
  select(-data)
common_beacons
...
```{r}
common_beacons %>%
  left_join(scanners, by = c("i", "scanner"))
```
...
```{r}
summarise(j = map2(j, j2, ~table(c(.x, .y))))
  summarise(j = map2_int(j, j2, ))
test %>%
  filter(i == 1) %>%
  unnest(j)
```
