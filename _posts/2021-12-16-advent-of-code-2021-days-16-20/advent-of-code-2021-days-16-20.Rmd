...
```{r}
add_snail <- function(l1, l2) {
  c(l1, l2)
}

get_explode_pair <- function(nl) {
  ei <- purrr::detect_index(nl, ~vec_depth(.x) == 4)
  
  if (ei > 0) {
    ep <- pluck(nl, ei)
    
    # Get the other indices that specify the exploding pair
    ei <- c(ei, detect_index(ep, ~vec_depth(.x) == 3))
    ep <- pluck(ep, ei[2])
    ei <- c(ei, detect_index(ep, ~vec_depth(.x) == 2))
    ep <- pluck(ep, ei[3])
    ei <- c(ei, detect_index(ep, ~length(.x) > 1))
    ep <- pluck(ep, ei[4])
...
pluck(sn, ei[1], ei[2], ei[3], ei[4])
pluck(sn, ei[1:3], ei[4] - 1)
```
...
explode_pair <- function(sn, res) {
  ...
  if (!is.null(pluck(sn, ei[1], ei[2], ei[3], ei[4] + 1))) {
    pluck(sn, ei[1], ei[2], ei[3], ei[4] + 1) <-
      pluck(sn, ei[1], ei[2], ei[3], ei[4] + 1) + ep[2]
  } else if (!is.null(pluck(sn, ei[1], ei[2], ei[3] + 1))) {
    pluck(sn, ei[1], ei[2], ei[3] + 1) <-
      pluck(sn, ei[1], ei[2], ei[3] + 1) + ep[2]
  } else if (!is.null(pluck(sn, ei[1], ei[2] + 1))) {
    pluck(sn, ei[1], ei[2] + 1) <-
      pluck(sn, ei[1], ei[2] + 1) + ep[2]
  } else if (!is.null(pluck(sn, ei[1] + 1))) {
    pluck(sn, ei[1] + 1) <-
      pluck(sn, ei[1] + 1) + ep[2]
  }
  
  if (ei[4] > 1) {
    pluck(sn, ei[1], ei[2], ei[3], ei[4] - 1) <-
      pluck(sn, ei[1], ei[2], ei[3], ei[4] - 1) + ep[1]
  } else if (ei[3] > 1) {
    pluck(sn, ei[1], ei[2], ei[3] - 1) <-
      pluck(sn, ei[1], ei[2], ei[3] - 1) + ep[1]
  } else if (ei[2] > 1) {
    pluck(sn, ei[1], ei[2] - 1) <-
      pluck(sn, ei[1], ei[2] - 1) + ep[1]
  } else if (ei[1] > 1) {
    pluck(sn, ei[1] - 1) <-
      pluck(sn, ei[1] - 1) + ep[1]
  }
  
  pluck(sn, ei[1], ei[2], ei[3], ei[4]) <- 0
  pluck(sn, ei[1], ei[2], ei[3]) <- unlist(pluck(sn, ei[1], ei[2], ei[3]))
  sn
}
```
...
```{r}
# Find the left-most or right-most value's index of a nested list
find_adjacent_idx <- function(sn, ei, side = c("L", "R")) {
  if (side == "L") {
    while (tail(ei, 1) == 1) {
      ei <- head(ei, -1)
      if (length(ei) == 0) return(ei)
    }
  } else if (side == "R") {
    whil
  }
...
explode_pair <- function(sn, res) {
  ...
  if (!is.null(pluck(sn, !!!ei[1:3], ei[4] + 1))) {
    idx <- find_adjacent_idx(sn, ei[1] + 1, side = "R")
    
    pluck(sn, !!!ei[1:3], ei[4] + 1) <-
      pluck(sn, !!!ei[1:3], ei[4] + 1) + ep[2]
  } else if (!is.null(pluck(sn, !!!ei[1:2], ei[3] + 1))) {
    pluck(sn, !!!ei[1:2], ei[3] + 1) <-
      pluck(sn, !!!ei[1:2], ei[3] + 1) + ep[2]
  } else if (!is.null(pluck(sn, !!!ei[1:2] + 1))) {
    pluck(sn, ei[1], ei[2] + 1) <-
      pluck(sn, ei[1], ei[2] + 1) + ep[2]
  } else if (!is.null(pluck(sn, ei[1] + 1))) {
    idx <- find_adjacent_idx(sn, ei[1] + 1, side = "R")
    pluck(sn, !!!idx) <- pluck(sn, idx) + ep[2]
  }
...
ex2_nl <- "[7,[6,[5,[4,[3,2]]]]]"
ex3_nl <- "[[6,[5,[4,[3,2]]]],1]"
ex4_nl <- "[[3,[2,[1,[7,3]]]],[6,[5,[4,[3,2]]]]]"
sn <- ex4_nl %>% fromJSON()

# Look for an exploding pair
res <- get_explode_pair(sn)
if (!is_empty(res)) {
  # Explode it and return the result
  snailfish_num <- explode_pair(sn, res)
}
snailfish_num
```
...
```{r}
ep <- get_explode_pair(test)
ei <- ep[[1]]
ep <- ep[[2]]

# Find the regular number to the left of the exploding pair
if (ei[3] > 1) {
  
  test %>% pluck(ei[1]) %>% pluck(ei[2]) %>% pluck(ei[3])
  test[[ei[1]]][[ei[2]]][[ei[3] - 1]] <-
    test[[ei[1]]][[ei[2]]][[ei[3] - 1]] + ep[1]
} else if (ei[2] > 1) {
  test[[ei[1]]][[ei[2] - 1]] <-
    test[[ei[1]]][[ei[2] - 1]] + ep[1]
}
} else if (ei[1] > 1) {
  test[[ei[1]]][[ei[2] - 1]] <-
    test[[ei[1]]][[ei[2] - 1]] + ep[1]
...
```{r}
ei
    explode_idx <- c(explode_idx,
                     detect_index(nl, ~vec_depth(.x) == vd))
    
    for (vd in seq(3, 1)) {
      nl <- nl[[tail(explode_idx, 1)]]
    }
  ...
  return(list(explode_idx, nl))
}
```


```{r}
get_explode_idx(test)
```
...
explode_idxs <- c(
      explode_idx,
      purrr::detect_index(test[[explode_idx]], ~vec_depth(.x) == 4),
      purrr::detect_index(test[explode_idx], ~vec_depth(.x) == 4),
...
test <- list(list(list(1, 2)), list(3, 4), list(list(5, 6)))
test <- list(list(list(1, 2)), list(3, 4), list(list(5, list(6, 7)), list(8, 9)))

explode_idx <- purrr::detect_index(test, ~vec_depth(.x) == 4)

map_depth(test[explode_idx], 5, ~.x*2)
explode_pair <- unlist(test[explode_idx], recursive = FALSE)
```
...
explode_idx <- purrr::detect_index(test, ~vec_depth(.x) == 4)
if (explode_idx > 0) {
  # Flatten the exploded pair
  map_depth(test[explode_idx], 2, ~ 0)
}

# Look for any pairs at 4 levels of nesting
depths <- map_int(test, vec_depth)
if (any(depths >= 3)) {
  # Explode just the first pair
  explode_idx <- which(explode_pair == 3)[[1]]
  exploding_pair <- map_depth
  test[explode_idx] <- map_depth(test[explode_idx], .depth = 2, unlist)
