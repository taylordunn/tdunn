...
For each scanner, I want a function that will give me all possible orientations

```{r}
# Multiplication factors to orient the (x,y,z) positions in 3d space
directions <-
  crossing(mx = c(-1, 1), my = c(-1, 1), mz = c(-1, 1))

get_rotations <- function(x, y, z, choice = NULL) {
  xyz <- combinat::permn(c(x, y, z)) %>%
    enframe(name = "perm") %>%
    unnest_wider(value, names_sep = "_") %>%
    rename(x = value_1, y = value_2, z = value_3)
  
  crossing(directions, xyz) %>%
    mutate(x = mx * x, y = my * y, z = mz * z) %>%
    distinct()
}
```

I want to generate a "blueprint" for each scanner that uniquely identifies it with relative positions of beacons.
To do this, I will compute the pairwise distances ($x^2 + y^2 +z^2$) between each beacon for a single scanners.
For 25 beacons per scanner, this means a total number of combinations given by "25 choose 2":
...
scanners_pair_dist <- scanners %>%
  ...
  map_dfr(
    ~crossing(
      ...
      select(i = i1, i2, dist2) %>%
      group_by(i) %>%
      nest(pair_dist = c(i2, dist2)),
    .id = "scanner"
  ) %>%
  ungroup() %>%
  mutate(scanner = as.integer(scanner))
scanners_pair_dist
```


```{r}
scanners_overlap <- scanners_pair_dist %>%
  unnest(pair_dist) %>%
  nest_by(scanner) %>%
  crossing(
    rename_with(., ~paste0(.x, "2"))
  ) %>%
  filter(scanner < scanner2) %>%
  mutate(
    overlap = map2(
      data, data2,
      function(d, d2) {
        d2 <- d2 %>% rename(j = i, j2 = i2)
        inner_join(d, d2, by = "dist2")
      }
    ),
    n_overlap = map_int(overlap, nrow)
  ) %>%
  select(-data, -data2)
scanners_overlap
```
...
Then for scanner 0, we find the other scanner with the largest overlap is scanner 1:

```{r}
d <- scanners_overlap %>% filter(scanner == 0, n_overlap == max(n_overlap))
d
```

Then find the overlapping points from each scanner:

```{r}
overlap_points <- d %>%
  unnest(overlap) %>%
  select(scanner, scanner2, i, j, j2) %>%
  pivot_longer(cols = c(j, j2)) %>%
  group_by(scanner, scanner2, i, value) %>%
  tally() %>%
  slice_max(n) %>%
  ungroup() %>%
  select(scanner, scanner2, i, j = value) %>%
  left_join(
    scanners %>% rename(xi = x, yi = y, zi = z),
    by = c("i", "scanner")
  ) %>%
  left_join(
    scanners %>% rename(j = i, xj = x, yj = y, zj = z, scanner2 = scanner),
    by = c("j", "scanner2")
  )
overlap_points
```

Now I have to determine the correct orientation of scanner 2:

```{r}
overlap_points %>%
  select(xj, yj, zj) %>%
  mutate(
    test = pmap(list(xj, yj, zj), get_rotations) 
  ) %>%
  unnest(test)
```
...
relative_pos <- overlap_points %>%
  mutate(dx = xi - xj, dy = yi - yj, dz = zi - zj)
...
```{r}
path <- "_posts/2021-12-16-advent-of-code-2021-days-16-20/day19-input-example.txt"
path <- "day19-input-example.txt"
