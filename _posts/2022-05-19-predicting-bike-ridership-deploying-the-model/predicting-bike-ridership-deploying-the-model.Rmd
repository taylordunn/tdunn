...
For instance, you can get a prediction for `n_bikes` on Hollis St tomorrow (May 23rd, 2022) with the following R code:
...
This could include data validation (e.g. trigger an alert if new data have abnormal or missing values) and model drift detection (e.g. model performance is below some threshold which triggers model re-tuning).
...
The packages by Mark Edmondson ([`googleCloudStorageR`](https://code.markedmondson.me/googleCloudStorageR/), [`googleCloudRunner`](https://code.markedmondson.me/googleCloudRunner/), [`bigQueryR`](https://code.markedmondson.me/bigQueryR/)), and the accompanying documentation, were a great help.
