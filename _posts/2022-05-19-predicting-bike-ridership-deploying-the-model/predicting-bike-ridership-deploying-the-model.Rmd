...
For instance, you can get a prediction for tomorrow's `n_bikes` on Hollis St with the following R code:
...
```{r code_folding="app.R", eval=FALSE}
library(shiny)
...
I set up the [Docker container](https://github.com/taylordunn/hfx-bike-ridership/blob/main/app/Dockerfile), pushed it to Container Registry, and deployed on Cloud Run.
...
<iframe height="900" width="100%" frameborder="no" src="https://hfx-bike-ridership-app-74govvz7xq-uc.a.run.app/" class="l-screen-inset shaded"> </iframe>
...
To add some interactivity to this app, I decided to make this part editable.
The weather variables in the table at bottom can be double-clicked and changed.
...
In summary, in this post I used Google Cloud Platform to automate the data ETL pipeline and model training.
Then model was then deployed in a REST API and Shiny dashboard.
When considering the full MLOps cycle, the biggest missing piece is some sort of monitoring functionality.
This could include data validation (e.g. alert is triggered if new data have abnormal or missing values) and model drift detection (e.g. model performance is below some threshold which triggers model re-tuning).
...
The packages made my Mark Edmondson ([`googleCloudStorageR`](https://code.markedmondson.me/googleCloudStorageR/), [`googleCloudRunner`](https://code.markedmondson.me/googleCloudRunner/), [`bigQueryR`](https://code.markedmondson.me/bigQueryR/)) were also a great help.
...
I hope that my write-ups and code can help others in getting their own projects up-and-running.
[David Robinson put it best in his 2019 keynote](https://www.rstudio.com/resources/rstudioconf-2019/the-unreasonable-effectiveness-of-public-work/):
