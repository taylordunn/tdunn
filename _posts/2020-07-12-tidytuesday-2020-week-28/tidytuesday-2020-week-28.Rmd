...
Yes, that looks to be the range of altitudes with the highest ratings on average.

## Model

I want to attempt to predict scores for one of the individual gradings (not the 0-100 total score, or the three that are mostly 10s), but can't decide, so choose randomly:
...
coffee_resamples <- bootstraps(coffee_train, strata = flavor)
```
...
coffee_rec <-
  recipe(
    flavor ~
      ...
      cupper_points + moisture + category_one_defects + category_two_defects +
      quakers + altitude_mean_meters_fixed,
    data = coffee_train
  ) %>%
  # Where missing, impute categorical variables with the most common value
  #step_impute_mode(all_nominal_predictors(), -variety) %>%
  ...
  # 4 degrees of freedom should give 3 knots
...
collect_predictions(ranger_fit) %>%
  ggplot(aes(flavor, .pred)) +
  geom_point(color = coffee_pal[1], alpha = 0.5) +
  geom_abline(color = coffee_pal[2], size = 1.5)
```
...
```{r}
set.seed(23)
coffee_sample <- coffee %>% slice_sample(n = 1)

coffee_sample %>% glimpse()
```

The `flavor` rating is `r coffee_sample$flavor`.
Our model predicts:

```{r}
predict(ranger_fit$.workflow[[1]], coffee_sample)
```
...
Define the workflow for a LASSO regression model:

```{r}
coffee_rec <-
  recipe(
    flavor ~ species + country_of_origin + processing_method + color +
      in_country_partner + variety + aroma + aftertaste +
      acidity + body + balance + uniformity + clean_cup + sweetness +
      cupper_points + moisture + category_one_defects + category_two_defects +
      quakers + altitude_mean_meters_fixed,
    data = coffee_train
  ...
  # Some of these categorical variables have too many levels, group levels
  #  with <5% frequency 
  # step_other(country_of_origin, variety, in_country_partner,
  #            processing_method, threshold = 0.05) %>%
...
lasso_fit_train <- lasso_workflow %>%
...
lasso_fit_train %>%
  ...
  ggplot(aes(step, estimate, group = term)) +
  geom_line()
```
...
```{r}
coffee_subset <- coffee %>%
  select(
    flavor, aroma, aftertaste, acidity, body, balance, uniformity, clean_cup, sweetness, cupper_points, moisture,
    category_one_defects, category_two_defects, quakers, altitude_mean_meters_fixed
  )
set.seed(42)
coffee_split <- initial_split(coffee_subset, prop = 3/4, strata = flavor)
coffee_train <- training(coffee_split)
coffee_test <- testing(coffee_split)

#coffee_resamples <- vfold_cv(coffee_train, v = 5, strata = flavor)
coffee_resamples <- bootstraps(coffee_train, strata = flavor)
coffee_rec_numerical <-
  recipe(
...
lasso_workflow <- workflow() %>%
  add_recipe(coffee_rec_numerical) %>%
  add_model(lasso_spec)
...
# Can't seem to get LASSO working properly
```
