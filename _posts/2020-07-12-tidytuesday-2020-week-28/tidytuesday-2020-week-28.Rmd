...
Load `tidymodels`, split the data 3:1 into training and testing (stratified by the outcome `flavor`), and define the cross-validation strategy:
...
coffee_splits <- vfold_cv(coffee_train, v = 5, strata = flavor)
```

Now define the recipe:

* Impute the mode for missing categorical values
* Lump together categorical values with <10% frequency
...
coffee_rec <-
  recipe(
    flavor ~ species + country_of_origin + processing_method + color +
      in_country_partner + variety + aroma + flavor + aftertaste +
  ...
  step_other(country_of_origin, variety, in_country_partner,
             processing_method, threshold = 0.10) %>%
  # Finally, create the dummy variables
...
Load `tidymodels`, setup parallel processing, and convert characters to factors:
